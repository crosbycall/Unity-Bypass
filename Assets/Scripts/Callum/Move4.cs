...
public class Move4 : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (!useController)
        {
            ...
            //Code for shooting
            float fire = Input.GetAxis("Fire4");
        ...
        if (useController)
        {
            float moveHorizontal = Input.GetAxis("Horizontal4");
            float moveVertical = Input.GetAxis("Vertical4");
            float lookHorizontal = Input.GetAxis("Right_Horizontal4");
            float lookVertical = Input.GetAxis("Right_Vertical4");
            ...
            if (fire < 1 && chargeLevel < 8)//If we release fire too early, don't do anything except reset chargelevel to 0.
            {
                midDash = false;
                Bullet.transform.localScale = initSize;
                chargeLevel = 0;
...
////Code for shooting the original arc shot
